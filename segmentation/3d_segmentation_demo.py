...
def read_image(test_data_path, data_name, sizes=(128, 128, 128), is_series=True,
    ...
    if is_series:
        ...
        for j, s in enumerate(data_seq):
            ...
            image = image_handle.get_fdata().astype(np.float32)
        ...
        data = data_handle.get_fdata().astype(np.float32)
...
def main():
    ...
    if args.output_nifti and is_nifti_data:
        for seg_res in list_seg_result:
            nii_filename = os.path.join(args.path_to_output, 'output_{}.nii.gz'.format(os.path.basename(args.path_to_input_data)))
